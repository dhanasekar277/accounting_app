...
from apps.expences.models import Expences, Assets, ExpenceCategories
...
def BalanceSheet(request):
    ...
    if request.GET.get('calander_from'):
        ...
        # DUTIES & TAXES
        # Input (Purchase)
        voucher_input = VoucherDetail.objects.filter(quotation__agency=active_agency,Date__range=[fromdate,todate]).values_list('quotation__order__Materials__Product_Name__GST').distinct()
        # print(voucher_input)
        voucher_sgst_cgst_amt = []
        total_voucher_sgst_cgst_amt = 0
        for i in voucher_input:
            for j in i:
                voucher_with_gst_amount = VoucherDetail.objects.filter(quotation__agency=active_agency,Date__range=[fromdate,todate],quotation__order__Materials__Product_Name__GST=j).distinct().aggregate(Sum('amount__total_amount')).get('amount__total_amount__sum') or 0
                gst = int(j)
                gst_amount = round((gst * voucher_with_gst_amount)/100)
                total_voucher_sgst_cgst_amt += gst_amount
                voucher_sgst_cgst_amt.append({'sgst_cgst_percent':gst/2,'sgst_cgst_value':gst_amount/2})

        # Output (Sales)
        sales_output = SalesInvoice.objects.filter(saleid__orderid__Agency=active_agency,Date__range=[fromdate,todate]).values_list('invoice__orderid__Materials__Product_Name__GST').distinct()
        # print(sales_output)
        sales_sgst_cgst_amt = []
        total_sales_sgst_cgst_amt = 0
        for i in sales_output:
            for j in i:
                sales_with_gst_amount = SalesInvoice.objects.filter(saleid__orderid__Agency=active_agency,Date__range=[fromdate,todate],invoice__orderid__Materials__Product_Name__GST=j).distinct().aggregate(Sum('saleid__sales__netAmount__WithGSTNetTotalAmount')).get('saleid__sales__netAmount__WithGSTNetTotalAmount__sum') or 0
                gst = int(j)
                gst_amount = round((gst * sales_with_gst_amount)/100)
                total_sales_sgst_cgst_amt += gst_amount
                sales_sgst_cgst_amt.append({'sgst_cgst_percent':gst/2,'sgst_cgst_value':gst_amount/2})
        ...
        context['currentassets'] = Assets.objects.filter(Agency=active_agency,Date__range=[fromdate,todate],Asset_Type="Current Asset")
        context['fixedassets'] = Assets.objects.filter(Agency=active_agency,Date__range=[fromdate,todate],Asset_Type="Fixed Asset")
        # Duties & Taxes
        context['input_sgst_cgst_amt'] = voucher_sgst_cgst_amt
        context['output_sgst_cgst_amt'] = sales_sgst_cgst_amt
        # Totals
        ...
        context['total_current_assets'] = Assets.objects.filter(Agency=active_agency,Date__range=[fromdate,todate],Asset_Type="Current Asset").aggregate(Sum('Asset_Value'))
        context['total_fixed_assets'] = Assets.objects.filter(Agency=active_agency,Date__range=[fromdate,todate],Asset_Type="Fixed Asset").aggregate(Sum('Asset_Value'))
        # Sundry creditors
        ...
        # Duties & Taxes
        context['input_sgst_cgst_total'] = total_voucher_sgst_cgst_amt
        context['output_sgst_cgst_total'] = total_sales_sgst_cgst_amt

    return render(request, 'balancesheet/balancesheet.html',context)
...
def ExpenceLedger(request, expcategory, fromdate, todate):
    ...
    particular_expence = serializers.serialize(
        'json',
        Expences.objects.filter(Agency=active_agency,Date__range=[fromdate,todate],Payment_Type='In Direct',Expence_Category__Category=expcategory), 
...
def DirectExpenceLedger(request, expcategory, fromdate, todate):
    ...
    particular_direct_expence = serializers.serialize(
        'json',
        Expences.objects.filter(Agency=active_agency,Date__range=[fromdate,todate],Payment_Type='Direct',Expence_Category__Category=expcategory), 
        use_natural_foreign_keys=True,
        fields = ('Expence_Category','Amount','Payment_Mode','Date')
    )
...
def SalesLedger(request, month):
    ...
    queryset = SalesInvoice.objects.filter(saleid__orderid__Agency=active_agency,Date__month=month)
...
def PurchaseLedger(request, month):
    ...
    custom_serializers = CustomSerializer()
    queryset = VoucherDetail.objects.filter(quotation__agency=active_agency,Date__month=month)
    total = VoucherDetail.objects.filter(quotation__agency=active_agency,Date__month=month).aggregate(Sum('amount__total_amount'))
